...
export default function UtilityCheck({params} : {params: {id: string}}) {
    ...
    const handleCheck = async (utility: Utility) => {
        if (booking?.utility && booking?.utility.length > 0) {
            setUtilityPrice({
                plumbing: (utility.plumbing - booking.utility[0].plumbing) * 20,
                electricity: (utility.electricity - booking.utility[0].electricity) * 7
            });
        }
    } 
	...
	return (
		<div>
			...
			<div className="container mx-auto mb-44">
				...
				{ booking?.utility && booking?.utility.length > 0 ? <div>
                    <div className="my-2">
                        <div className="text-xl font-semibold">หน่วยของเดือนล่าสุด</div>
                        <span className="font-medium">ค่าน้ำประปา</span> : { booking.utility[0].plumbing } หน่วย<br />
                        <span className="font-medium">ค่าไฟ</span> : { booking.utility[0].electricity } หน่วย<br />
                    </div>
                    <div className="text-xl font-semibold mt-3">ใส่หน่วยปัจจุบันเพื่อคำนวณ</div>
                    <CheckUtilityForm  handleCheck={handleCheck} />
                    { 
                        utilityPrice.electricity !== -1 && <div className="mt-2">
                            <span className="font-medium">ค่าน้ำประปา</span> : { utilityPrice.plumbing } บาท<br/>
                            <span className="font-medium">ค่าไฟ</span> : { utilityPrice.electricity } บาท
                        </div>
                    }
                </div> :
                <div>
                    <span className="text-red-500">กรุณารอเจ้าหน้าที่กำหนดค่าไฟฟ้า และ ค่าน้ำ</span>
                </div>
                }
			</div>
		</div>
