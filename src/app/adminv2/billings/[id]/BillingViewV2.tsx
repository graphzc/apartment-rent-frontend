...
const BillingViewV2 = ({ id }: BillingViewV2Props) => {
  ...
  const handleStatusChange = async (newStatus: "PENDING" | "PAID" | "FAILED") => {
  ...
  const getTypeInfo = (type: string) => {
    const typeMap = {
      FIRST_TIME_BOOKING: { color: "text-blue-800 bg-blue-100", text: "จองครั้งแรก" },
      MONTHLY_RENT: { color: "text-purple-800 bg-purple-100", text: "ค่าเช่ารายเดือน" },
      UTILITY: { color: "text-orange-800 bg-orange-100", text: "ค่าสาธารณูปโภค" },
    };
        ...
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <span className={`inline-flex px-3 py-2 rounded-full text-sm font-medium ${typeInfo.color}`}>
                {typeInfo.text}
              </span>
            </div>
          </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div>
                <h2 className="text-lg font-medium text-gray-900 mb-4">ข้อมูลบิล</h2>
                <div className="space-y-4">
                  <div className="flex items-start">
                    <DocumentTextIcon className="h-5 w-5 text-gray-400 mt-1 mr-3" />
                    <div>
                      <p className="text-sm font-medium text-gray-900">เลขที่บิล</p>
                      <p className="text-gray-600">#{billing.billingReference}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">จำนวนเงิน</p>
                      <p className="text-2xl font-bold text-green-600">{formatCurrency(billing.amount)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">กำหนดชำระ</p>
                      <p className="text-gray-600">{formatDate(billing.dueDate)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start">
                    <CalendarIcon className="h-5 w-5 text-gray-400 mt-1 mr-3" />
                    <div>
                      <p className="text-sm font-medium text-gray-900">วันที่สร้างบิล</p>
                      <p className="text-gray-600">{formatDate(billing.createdAt)}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Status Management */}
              <div>
                <h2 className="text-lg font-medium text-gray-900 mb-4">จัดการสถานะ</h2>
                <div className="space-y-3">
                  ...
                  <div className="flex space-x-3">
                    {["PENDING", "PAID", "FAILED"].map((status) => (
                      <button
                        key={status}
                        onClick={() => handleStatusChange(status as "PENDING" | "PAID" | "FAILED")}
                        disabled={updateStatusMutation.isPending || billing.status === status}
                        ...
                        {payment.slipImage && (
                          <div className="mt-3">
                            <div className="flex items-center mb-2">
                              ...
                              <span className="text-sm text-gray-700">หลักฐานการชำระเงิน</span>
                            </div>
                            <div className="relative">
                              <img
                                ...
                                onClick={() => window.open(payment.slipImage, '_blank')}
                              />
                            </div>
                          </div>
                        )}
